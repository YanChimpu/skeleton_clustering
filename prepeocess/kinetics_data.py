...
class Feeder_kinetics(Dataset):
    ...
    def load_data(self):
        ...
        f.close()
        sample_id = [name.split(".")[0] for name in self.sample_name][:40000]
        self.label = []
        has_skeleton = []
        for id in sample_id:
            if len(id) == 11:
                self.label.append(label_info[id]["label_index"])
                has_skeleton.append(label_info[id]["has_skeleton"])
...
if __name__ == '__main__':
    data_path = "/Users/pu/Documents/work/data/kinetics_skeleton/kinetics-skeleton"
    out_folder = "."
    # part = ["val", "train"]
    part = ["train"]
    # part = ["val"]
    for p in part:
        ...
        data_path_ = "{}/kinetics_{}".format(data_path, p)
        label_path = '{}/kinetics_{}_label.json'.format(data_path, p)
        data_out_path = '{}/{}_data_joint.npy'.format(out_folder, p)
        label_out_path = '{}/{}_label.pkl'.format(out_folder, p)

        gendata(data_path_, label_path, data_out_path, label_out_path)
